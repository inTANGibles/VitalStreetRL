...
def personal_3D_path (wifi_df,devices_axis_df):
    ...
    save_path = r"D:\postgraduate study\互动建筑设计\东吴校园数据挖掘\3d_path2"
    str_mac_tuple = wifi_df["m"].apply(lambda x : tuple(x)).unique()
    print(len(str_mac_tuple))

    for i, str_mac in enumerate(tqdm(str_mac_tuple, desc='Saving Plots', unit='plot')):
        ...
        #保存图片
        save_file = f'{save_path}\\3d_path_{i}.png'
        plt.axis('off')
