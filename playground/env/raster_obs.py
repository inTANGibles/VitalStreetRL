    ...
    def encode(self, state: WorldState) -> np.ndarray:
        ...
        for i, channel_name in enumerate(self.channels):
            try:
                ...
                print(f"[错误] 渲染通道 {channel_name} 失败: {e}, 使用空通道")
                # 使用空通道作为fallback
                H, W = self.resolution
                empty_channel = np.zeros((H, W), dtype=np.float32)
                obs_channels.append(empty_channel)
        
        # 堆叠为多通道数组 (C, H, W)
        try:
            obs = np.stack(obs_channels, axis=0).astype(np.float32)
        except Exception as e:
            print(f"[错误] 堆叠通道失败: {e}, 使用默认观测")
            # 使用默认观测
            H, W = self.resolution
            obs = np.zeros((len(self.channels), H, W), dtype=np.float32)
        ...
        if channel_name == 'walkable_mask':
            ...
            for idx, geometry, flow in unit_geometries:
                try:
                    ...
                    print(f"[警告] 栅格化单元 {idx} (flow={flow}) 失败: {e}, 跳过")
                    continue
            ...
            # 非shop单元类型的映射（atrium和circulation都视为public_space）
            unit_type_to_gray = {
                'atrium': 0.0,  # 视为public_space
                'public_space': 0.0,
                'circulation': 0.0,  # 视为public_space
            }
            ...
            for idx, unit in space_units.iterrows():
                try:
                    ...
                    print(f"[警告] 栅格化单元 {idx} (landuse) 失败: {e}, 跳过")
                    continue
